...
void MyGui::initSignalsAndSlots()
{
	...
	QObject::connect(Mylist, &QListWidget::itemSelectionChanged, [&]() {
		...
		QListWidgetItem  *selItem = Mylist->selectedItems().at(0);
		QString txt = selItem->text();
...
void MyGui::FilterByTypeGUI()
{
	...
	if (ok && !text.isEmpty()) {
		...
		vector<Activity> activities = serv.filterByString(txt, getType);
		for (const auto& act : serv.getAll()) {
			auto it = find_if(activities.begin(), activities.end(), [act](const Activity& oth) { return act == oth; });
